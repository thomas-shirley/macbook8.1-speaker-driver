...
static int cs_init(struct hda_codec *codec)
{
	...
	if (spec->is_macbook81) {
		/* Drive the codec into the exact EFI chime state. The charge
		 * pump (coef 0x00 bit 14) is deliberately NOT enabled: EFI's
		 * working coef 0x00 = 0x00c4 has it clear, and setting it knocks
...
static const struct hda_quirk cs4208_mac_fixup_tbl[] = {
	...
	HDA_CODEC_QUIRK(0x106b, 0x6400, "MacBook8,1", CS4208_MB81),
			      ...
			      const struct hda_fixup *fix, int action)
{
	if (action == HDA_FIXUP_ACT_PRE_PROBE) {
		...
		/* MacBook8,1: GPIO0=mute(active-high) is the ONLY pin the codec
		 * drives. GPIO4/GPIO5 (CH/US DFET amp rails) are left as EFI set
		 * them — driving/power-cycling them bricks the class-D amp (see
		 * mb81_dfet_power_up). gpio_eapd_speaker triggers the
		 * is_macbook81 branch in cs_automute (which only writes GPIO0). */
		spec->gpio_eapd_hp = 0;
		spec->gpio_eapd_speaker = 1;
		spec->gpio_mask = 0x01;
		spec->gpio_dir  = 0x01;
			     ...
			     const struct hda_fixup *fix, int action)
{
	...
	/* DMI fallback: SSID may be lost after driver reload on Apple hardware */
	if (codec->fixup_id == HDA_FIXUP_ID_NOT_SET &&
	    dmi_match(DMI_PRODUCT_NAME, "MacBook8,1"))
		codec->fixup_id = CS4208_MB81;
	if (codec->fixup_id == HDA_FIXUP_ID_NOT_SET)
...
static int cs4208_probe(struct hda_codec *codec)
{
	...
	if (codec->core.subsystem_id == 0x106b6400 ||
	    dmi_match(DMI_PRODUCT_NAME, "MacBook8,1")) {
		/* MacBook8,1: TDM path via 0x0a → 0x1d, class-D amp via DFET GPIOs.
		 * setup_macbook81 overrides pin configs and coefficients so the
		 * auto-parser sees 0x1d (TDM speaker) instead of 0x11-0x14 (analog).
		 * DMI fallback handles driver reload (SSID reset by HDA controller). */
		spec->is_macbook81 = 1;
		err = setup_macbook81(codec);
		if (err < 0)
			return err;
	} else {
		/* MacBook9,1 / 10,1 and other CS4208: A1534 explicit analog path */
		spec->is_macbook81 = 0;
